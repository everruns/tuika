...
Usage:
    ...
    python3 benches/check_iai.py            # check (exit 1 on regression)
    python3 benches/check_iai.py --update   # (re)write the baselines
...
def write_baseline(crate_dir: Path, benchmarks: dict[str, int]) -> None:
    ...
    doc = {
        "_comment": (
            ...
            "`python3 benches/check_iai.py --update` and commit alongside the "
...
def check(by_crate: dict[Path, dict[str, int]]) -> int:
    ...
    for crate_dir, measured in sorted(by_crate.items(), key=lambda kv: str(kv[0])):
        ...
        if baseline is None:
            print(
                ...
                f"`python3 benches/check_iai.py --update` and commit it"
            )
            failed = True
            continue
    ...
    if failed:
        print(
            ...
            "`python3 benches/check_iai.py --update` and commit the baseline."
        )
        return 1
