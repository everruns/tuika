...
- A recording of a whole runnable example lives **beside that example**, so its
  ...
  The focused `AppShell` application recording lives beside its single-file
  example at `examples/app_shell.gif`, regenerated by
  `scripts/gen-app-shell-demo.sh` from the real `app_shell` binary in the shared
  Solarized Dark documentation palette.
