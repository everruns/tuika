//! Terminal-protocol smoke for the full-screen host.
//!
...
/// Locate the compiled `gallery` example.
///
...
/// (`target/<profile>/deps/<test>` → `target/<profile>/examples/gallery`).
fn gallery_bin() -> PathBuf {
    let test_bin = std::env::current_exe().expect("test binary path");
    let profile_dir = test_bin
        .parent()
        .and_then(|deps| deps.parent())
        .expect("target/<profile> directory");
    let bin = profile_dir.join("examples").join("gallery");
    assert!(
        bin.is_file(),
        "the `gallery` example was not built at {} — `cargo test` should build \
         examples; run `cargo build --example gallery` if invoking the test \
         binary directly",
        bin.display()
    );
    bin
...
/// Spawn the gallery under a pty of the given size, optionally resize mid-run,
/// then send `q` and collect everything the terminal received.
