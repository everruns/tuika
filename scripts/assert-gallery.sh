#!/usr/bin/env bash
# Assert that a captured terminal screen shows the `gallery` example rendered
...
capture="${1:?usage: assert-gallery.sh <capture-file>}"
if [[ ! -s "$capture" ]]; then
...
need "progress"
need "Braille"
need "https://github.com/everruns/tuika"
