#!/usr/bin/env bash
# Best-effort: drive the `gallery` example inside kitty (a GPU terminal) and
...
#     bash scripts/kitty-capture.sh "$PWD/target/debug/examples/gallery"
#
...
set -x
bin="${1:?usage: kitty-capture.sh <gallery-binary>}"
sock="unix:/tmp/kitty-$$"
